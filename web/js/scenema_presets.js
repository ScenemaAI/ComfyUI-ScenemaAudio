...
//
// When a user picks a preset on Scenema Audio Generate, populate the
// voice_description / gender / speech_text / scene / custom_scene /
// action_tags widgets. Preset definitions ported from scenema.ai/audio
// demos. Keep in sync with nodes/presets.py.

import { app } from "../../../scripts/app.js";

console.log("[Scenema Audio] preset extension loading");
...
function applyPreset(node, presetName) {
    console.log("[Scenema Audio] applyPreset:", presetName);
    ...
    for (const [fieldName, value] of Object.entries(preset)) {
        ...
        if (widget) {
            ...
            console.warn("[Scenema Audio] widget not found:", fieldName);
        }
    }
    console.log(`[Scenema Audio] updated ${updated} widgets`);
...
function hookNode(node) {
    ...
    if (node.__scenemaPresetHooked) return true;
    ...
    node.__scenemaPresetHooked = true;
    console.log("[Scenema Audio] hooked preset on node", node.id);
    return true;
}
...
function scanExistingNodes() {
    ...
    for (const node of nodes) {
        if (node.type === "ScenemaAudioGenerate") {
            hookNode(node);
        }
    }
}
...
app.registerExtension({
    name: "scenema.audio.presets",

    async setup() {
        console.log("[Scenema Audio] setup() called");
        setTimeout(scanExistingNodes, 500);
        ...
        if (origLoadGraphData) {
            app.loadGraphData = function () {
                ...
                setTimeout(scanExistingNodes, 100);
                return result;
            };
        }
    },
